...
var gl1 = {
	...
	drawImage: function(texPartX, texPartY, texPartSizeX, texPartSizeY, drawX, drawY, sizeX, sizeY, rgba, rotation) {
		...
		//
		if(this.clipY2) {
			if(drawY+sizeY < this.clipY1)return
			if(drawY > this.clipY2)return
			if(drawY+sizeY > this.clipY2) {
				if(texPartX || texPartY) {
					texPartSizeY *= (this.clipY2-drawY)/sizeY
				}
				sizeY = this.clipY2-drawY
			}
			if(drawY < this.clipY1) {
				//texPartSizeY *= (this.clipY2-drawY)/sizeY
				var add = this.clipY1-drawY
				var sizeY0 = sizeY
				sizeY -= this.clipY1-drawY
				drawY = this.clipY1
				if(texPartX || texPartY) {
	...
	setup: function(canvas, texFileName) {
		...
		// Tell gl where to read from our arrayBuffer to set our shader attibute variables each time an image is drawn.
		...
		if(texFileName) {
			...
			image.onload = function() {
				setTimeout(() => {
					...
					gl1.ready = true
					gLoadingDiv.style.display = 'none'
					setTimeout(gAudioSetup,44)
				},1)
			}
...
gl1.imageDraw = function(image, x, y, sizeX, sizeY, flipX, flipY, rot) {
	gl1.drawImage(image.texX, image.texY, image.sizeX, image.sizeY, this.offsetX+x, this.offsetY+y, sizeX||image.sizeX, sizeY||image.sizeY, image.rgb, rot)
}

gl1.rectDraw = function(drawX, drawY, sizeX, sizeY, rgba, rotation) {
	this.drawImage(0, 0, 1, 1, this.offsetX+drawX, this.offsetY+drawY, sizeX, sizeY, rgba, rotation)
}
